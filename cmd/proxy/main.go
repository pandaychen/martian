...
var (
	...
	/*
		addr           = flag.String("addr", ":8080", "代理的主机:端口")
		apiAddr        = flag.String("api-addr", ":8181", "配置API的主机:端口")
		tlsAddr        = flag.String("tls-addr", ":4443", "TLS上的代理的主机:端口")
		api            = flag.String("api", "martian.proxy", "API的主机名")
		generateCA     = flag.Bool("generate-ca-cert", false, "为MITM生成CA证书和私钥")
		cert           = flag.String("cert", "", "用于签名MITM证书的CA证书的文件路径")
		key            = flag.String("key", "", "用于签名MITM证书的CA的私钥的文件路径")
		organization   = flag.String("organization", "Martian Proxy", "MITM证书的组织名")
		validity       = flag.Duration("validity", time.Hour, "MITM证书有效的时间窗口")
		allowCORS      = flag.Bool("cors", false, "允许CORS请求配置代理")
		harLogging     = flag.Bool("har", false, "启用HAR日志API")
		marblLogging   = flag.Bool("marbl", false, "启用MARBL日志API")
		trafficShaping = flag.Bool("traffic-shaping", false, "启用流量整形API")
		skipTLSVerify  = flag.Bool("skip-tls-verify", false, "跳过TLS服务器验证；不安全")
		dsProxyURL     = flag.String("downstream-proxy-url", "", "下游代理的URL")
		level          = flag.Int("v", 0, "日志级别")
	*/
)
